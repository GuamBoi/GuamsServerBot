...
async def send_embed(ctx, message):
    embed = discord.Embed(description=message, color=discord.Color.random())  # Create embed with message and a random color
...
async def commands(ctx):
    ...
    embed = discord.Embed(title="Server Commands", color=0xff0000)
    embed.add_field(name="!dice_commands", value="lists all the dice commands", inline=False)
...
async def dice_commands(ctx):
    # Create an embed to display commands and descriptions
    embed = discord.Embed(title="Dice Commands", color=0xff0000)
    embed.add_field(name="!d4", value="Rolls a D4 Dice", inline=False)
    embed.add_field(name="!roll", value="Rolls a normal D6 dice", inline=False)
    embed.add_field(name="!d8", value="Rolls a D8 dice", inline=False)
    embed.add_field(name="!d10", value="Rolls a D10 dice", inline=False)
    embed.add_field(name="!d12", value="Rolls a D12 dice", inline=False)
    embed.add_field(name="!d20", value="Rolls a D20 dice", inline=False)
    await ctx.send(embed=embed)
...
async def d4(ctx):
    if D4_messages:  # If there are messages for D4
        ...
        await send_embed(ctx, response)  # Send the chosen message as an embed
    else:
        await send_embed(ctx, "No messages available for rolling a D4.")  # Send message if no D4 messages available
...
async def roll(ctx):
    if D6_messages:  # If there are messages for D6
        ...
        await send_embed(ctx, "No messages available for rolling a D6.")  # Send message if no D6 messages available
...
async def d8(ctx):
    if D8_messages:  # If there are messages for D8
        ...
        await send_embed(ctx, "No messages available for rolling a D8.")  # Send message if no D8 messages available
...
async def d10(ctx):
    if D10_messages:  # If there are messages for D10
        ...
        await send_embed(ctx, "No messages available for rolling a D10.")  # Send message if no D10 messages available
...
async def d12(ctx):
    if D12_messages:  # If there are messages for D12
        ...
        await send_embed(ctx, "No messages available for rolling a D12.")  # Send message if no D12 messages available
...
async def d20(ctx):
    if D20_messages:  # If there are messages for D20
        ...
        await send_embed(ctx, "No messages available for rolling a D20.")  # Send message if no D20 messages available
...
async def coinflip(ctx):
    if coinflip_messages:  # If there are messages for coinflip
        ...
        await send_embed(ctx, "No messages available for coin flipping.")  # Send message if no coinflip messages available
